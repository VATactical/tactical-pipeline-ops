...
const timezones = [
  ['America/Managua', 'Nicaragua · Managua'], ['Africa/Lagos', 'Nigeria · Lagos'],
  ['America/New_York', 'USA · Eastern'], ['America/Chicago', 'USA · Central'],
  ['America/Denver', 'USA · Mountain'], ['America/Los_Angeles', 'USA · Pacific'],
]
...
export default function TeamPage() {
  ...
  const [form, setForm] = useState({ email: '', password: '', fullName: '', role: 'onboarding_media', timezone: 'America/Managua', permissions: { ...defaultPermissions } })
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setForm({ email: '', password: '', fullName: '', role: 'onboarding_media', timezone: 'America/Managua', permissions: { ...defaultPermissions } })
  ...
  const saveProfile = async (profile) => {
    ...
    try {
      await manageTeamUser({ action: 'update', userId: profile.id, fullName: profile.full_name, role: profile.role, timezone: profile.timezone, permissions: profile.role === 'superadmin' ? allPermissions : profile.permissions, active: profile.active })
  ...
  return <div className="page-stack">
    ...
    <section className="content-card"><div className="section-heading"><div><p className="eyebrow">Nuevo acceso</p><h3>Crear usuario</h3></div></div><form className="user-create-form" onSubmit={submit}>
      ...
      <label>Zona horaria<select value={form.timezone} onChange={(event) => setFormField('timezone', event.target.value)}>{timezones.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
    ...
    <section className="team-grid">{profiles.map((member) => <article className={`content-card user-management-card ${member.active ? '' : 'inactive'}`} key={member.id}>
      ...
      <label>Zona horaria<select value={member.timezone || 'America/Managua'} onChange={(event) => changeProfile(member.id, 'timezone', event.target.value)}>{timezones.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
