...
export default function TeamPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="content-card"><div className="section-heading"><div><p className="eyebrow">{t('Equipo')}</p><h3>{t('Usuarios activos')}</h3></div><span className="status-pill">{profiles.filter((member) => member.active).length}</span></div><div className="team-grid">{profiles.filter((member) => member.active).map((member) => (<article className={`content-card user-management-card ${member.active ? '' : 'inactive'}`} key={member.id}>
      ...
      {resetUserId === member.id ? <div className="password-reset-panel"><label>Nueva contraseña<div className="inline-password"><input type={showResetPassword ? 'text' : 'password'} minLength="10" value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} /><button type="button" className="text-button" onClick={() => setShowResetPassword((value) => !value)}>{showResetPassword ? 'Ocultar' : 'Ver'}</button></div></label><p className="muted" aria-live="polite">{t('Mínimo 10 caracteres.')} <span>{resetPassword.length}/10</span></p><div className="section-actions"><button className="text-button" type="button" onClick={() => { setResetUserId(null); setResetPassword('') }}>Cancelar</button><button className="secondary-button" type="button" disabled={saving || resetPassword.length < 10} onClick={() => resetAccess(member)}>Restablecer</button></div></div> : <button className="text-button" type="button" onClick={() => { setResetUserId(member.id); setResetPassword('') }}>¿Perdió su contraseña? Restablecer</button>}
    </article>))}</div></section>
    <section className="content-card"><div className="section-heading"><div><p className="eyebrow">{t('Acceso')}</p><h3>{t('Usuarios inactivos')}</h3></div><span className="status-pill">{profiles.filter((member) => !member.active).length}</span></div><p className="muted">{t('Los usuarios inactivos no pueden acceder. Sus tareas, notas y actividad se conservan.')}</p><div className="team-grid">{profiles.filter((member) => !member.active).length === 0 ? <p className="muted">{t('No hay usuarios inactivos.')}</p> : profiles.filter((member) => !member.active).map((member) => (<article className={`content-card user-management-card ${member.active ? '' : 'inactive'}`} key={member.id}>
      <div className="section-heading"><div><strong>{member.full_name || 'Usuario sin nombre'}</strong><p className="muted">{member.email}</p></div><label className="active-toggle"><input type="checkbox" checked={member.active} onChange={(event) => changeProfile(member.id, 'active', event.target.checked)} />Activo</label></div>
      <label>Nombre<input value={member.full_name || ''} onChange={(event) => changeProfile(member.id, 'full_name', event.target.value)} /></label>
      <label>Rol<select value={member.role} onChange={(event) => changeProfile(member.id, 'role', event.target.value)}>{roles.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
      <label>Zona horaria<select value={member.timezone || 'America/Managua'} onChange={(event) => changeProfile(member.id, 'timezone', event.target.value)}>{timezones.map((zone) => <option value={zone} key={zone}>{zone}</option>)}</select></label>
