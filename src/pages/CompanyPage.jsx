...
import { loadCompanySettings, resolveCompanyLogo, saveCompanySettings, uploadCompanyLogo } from '../services/companyService'
...
export default function CompanyPage() {
  ...
  return <div className="page-stack compact-page"><header className="page-header"><div><p className="eyebrow">Administración</p><h2>Mi empresa</h2><p className="muted">Configura la identidad que aparece en la navegación, el footer y el navegador.</p></div></header>{error && <p className="form-error">{error}</p>}{message && <p className="form-success">{message}</p>}{form && <form className="content-card company-form" onSubmit={submit}><div className="company-logo-editor"><img src={resolveCompanyLogo(form.logo_url)} alt="Logo actual" /><label className="secondary-button">Cambiar logo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={uploadLogo} hidden /></label><small>PNG, JPG o WebP · máximo 5 MB</small></div><div className="company-fields"><label>Nombre del sistema<input value={form.system_name} onChange={(e) => setField('system_name', e.target.value)} required /></label><label>Empresa<input value={form.company_name} onChange={(e) => setField('company_name', e.target.value)} required /></label><label>Correo empresarial<input type="email" value={form.company_email} onChange={(e) => setField('company_email', e.target.value)} /></label><label>Sitio web<input type="url" value={form.company_website} onChange={(e) => setField('company_website', e.target.value)} placeholder="https://…" /></label><button className="primary-button compact-button" disabled={saving}>{saving ? 'Guardando…' : 'Guardar empresa'}</button></div></form>}</div>
}
