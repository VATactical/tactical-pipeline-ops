...
function TeamClocks({ members, currentId }) {
  ...
  return <section className="world-clocks">{members.map((member) => <article className={`clock-card ${member.id === currentId ? 'current' : ''}`} key={member.id}><span>{member.full_name}{member.id === currentId ? ' · Tú' : ''}</span><strong>{new Intl.DateTimeFormat('es', { timeZone: member.timezone, hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now)}</strong><small>{member.timezone.replaceAll('_', ' ')} · {new Intl.DateTimeFormat('es', { timeZone: member.timezone, weekday: 'short', month: 'short', day: 'numeric' }).format(now)}</small></article>)}</section>
}
...
export default function CalendarPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="content-card"><div className="section-heading"><div><p className="eyebrow">Próximos</p><h3>Agenda del equipo</h3></div></div><div className="calendar-list">{upcoming.length === 0 && <p className="muted">No hay eventos próximos.</p>}{upcoming.map((event) => <article className="calendar-event" key={event.id}><time>{new Intl.DateTimeFormat('es', { timeZone: displayZone, dateStyle: 'medium', timeStyle: 'short' }).format(new Date(event.start_at))}</time><div><strong>{event.title}</strong><p>{event.clients ? `${event.clients.code} · ${event.clients.business_name}` : 'Evento general'} · {event.event_type}</p><small>Zona del evento: {event.timezone}</small></div>{event.meeting_url && <a className="secondary-button" href={event.meeting_url} target="_blank" rel="noreferrer">Abrir enlace ↗</a>}</article>)}</div></section>
  </div>
}
