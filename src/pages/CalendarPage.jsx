...
export default function CalendarPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="content-card"><div className="section-heading"><div><p className="eyebrow">Próximos</p><h3>Agenda del equipo</h3></div></div><div className="calendar-list">{upcoming.length === 0 && <p className="muted">No hay eventos próximos.</p>}{upcoming.map((event) => <article className="calendar-event" key={event.id}><time>{new Intl.DateTimeFormat(locale, { timeZone: displayZone, dateStyle: 'medium', timeStyle: 'short' }).format(new Date(event.start_at))}</time><div><strong data-no-translate>{event.title}</strong><p><span data-no-translate>{event.clients ? `${event.clients.code} · ${event.clients.business_name}` : t('Evento general')}</span> · {t(event.event_type)}</p><small>Zona del evento: {event.timezone}</small></div>{event.meeting_url && <a className="secondary-button" href={event.meeting_url} target="_blank" rel="noreferrer">Abrir enlace ↗</a>}</article>)}</div></section>
  </div>
}
