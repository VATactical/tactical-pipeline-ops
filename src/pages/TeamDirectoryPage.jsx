...
export default function TeamDirectoryPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="directory-grid">{members.map((member) => {
      ...
      return <article className="content-card directory-card" key={member.id}><div className="profile-identity"><div className="brand-mark">{(member.full_name || 'TP').slice(0, 2).toUpperCase()}</div><div><h3>{member.full_name || 'Usuario'}</h3><span>{roleLabels[member.role] || member.role}</span></div></div><dl><div><dt>Correo</dt><dd>{member.email}</dd></div><div><dt>Slack</dt><dd>{member.slack_contact || 'Pendiente'}</dd></div><div><dt>WhatsApp</dt><dd>{member.whatsapp_contact || 'Pendiente'}</dd></div><div><dt>Zona horaria</dt><dd>{member.timezone}</dd></div></dl><div className="contact-actions">{member.id !== profile.id && <Link className="secondary-button message-user-button" to={`/mensajes?to=${member.id}`}>Mensaje</Link>}<a className="secondary-button" href={`mailto:${member.email}`}>Correo</a>{slack && <a className="secondary-button" href={slack} target="_blank" rel="noreferrer">Slack</a>}{whatsapp && <a className="secondary-button" href={whatsapp} target="_blank" rel="noreferrer">WhatsApp</a>}</div></article>
    })}</section>
  </div>
}
