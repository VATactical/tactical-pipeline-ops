...
const isMissing = (value, type) => {
  ...
  if (type === 'number') return value == null || value === '' || Number(value) <= 0
  return value == null || value === '' || /pendiente|confirmar|verificar|bloquead|rechazad/i.test(String(value))
}
...
export default function ClientDetailPage() {
  ...
  const missingFields = dossierFields
    .filter(([, key, type]) => isMissing(client[key], type))
    .map(([label]) => label)
