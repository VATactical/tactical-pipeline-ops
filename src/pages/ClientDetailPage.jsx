...
const sections = [
  ...
  { title: 'Infraestructura', fields: [
    ...
    ['Activos de marca / Drive · formulario', 'available_assets'], ['Subcuenta GHL creada', 'ghl_subaccount_created', 'boolean'], ['Carpeta Drive creada', 'drive_folder_created', 'boolean'],
  ] },
...
export default function ClientDetailPage() {
  ...
  const toggleIntakeForm = async () => {
    ...
    try {
      ...
      await refresh()
      setMessage(t(completed ? 'Formulario del cliente marcado como recibido.' : 'Formulario marcado como pendiente.'))
    } catch (updateError) { setError(t(updateError.message)) }
    finally { setSaving(false) }
  }
  const toggleSetupStatus = async (key) => {
    if (!canEdit) return
    setSaving(true); setError(''); setMessage('')
    try {
      await updateClient(clientId, { [key]: !client[key] })
      await refresh()
      setMessage(t(key === 'ghl_subaccount_created' ? 'Estado de subcuenta GHL actualizado.' : 'Estado de carpeta Drive actualizado.'))
    } catch (updateError) { setError(t(updateError.message)) }
    finally { setSaving(false) }
  }
  ...
  return <div className="page-stack client-dossier-page">
    ...
    {client.archived && <section className="content-card client-archive-summary"><div><p className="eyebrow">Cliente archivado</p><h3 data-no-translate>{client.archive_reason}</h3>{client.archive_note && <p data-no-translate>{client.archive_note}</p>}<small>{client.archived_at ? t(`Archivado el ${new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(client.archived_at))}`) : ''}</small></div>{canManageArchive && <button className="primary-button compact-button" type="button" disabled={saving} onClick={restoreClient}>{saving ? 'Restaurando…' : 'Restaurar cliente'}</button>}</section>}
    <section className="content-card dossier-command-center">
      <div className="section-heading"><div><p className="eyebrow">Configuración inicial</p><h3>Estado rápido del dossier</h3></div><span className="muted">{[client.ghl_subaccount_created, client.drive_folder_created].filter(Boolean).length}/2 completados</span></div>
      <div className="setup-status-grid">
        <button className={`setup-status-item ${client.ghl_subaccount_created ? 'complete' : 'pending'}`} type="button" disabled={!canEdit || saving} onClick={() => toggleSetupStatus('ghl_subaccount_created')}><span className="setup-status-icon">{client.ghl_subaccount_created ? '✓' : '1'}</span><span><strong>Subcuenta GHL</strong><small>{client.ghl_subaccount_created ? 'Creada' : 'Pendiente'}</small></span></button>
        <button className={`setup-status-item ${client.drive_folder_created ? 'complete' : 'pending'}`} type="button" disabled={!canEdit || saving} onClick={() => toggleSetupStatus('drive_folder_created')}><span className="setup-status-icon">{client.drive_folder_created ? '✓' : '2'}</span><span><strong>Carpeta Drive</strong><small>{client.drive_folder_created ? 'Creada' : 'Pendiente'}</small></span></button>
      </div>
      <div className="dossier-command-meta"><span><b>{missingFields.length}</b> campos pendientes</span><span><b>{client.target_launch_date || 'Sin fecha'}</b> lanzamiento</span><span><b>{client.next_action || 'Sin acción'}</b></span></div>
    </section>
    ...
    {['ADS LIVE', 'ADS PAUSED'].includes(client.status) && <AdsReportsPanel client={client} canEdit={canEdit} />}
